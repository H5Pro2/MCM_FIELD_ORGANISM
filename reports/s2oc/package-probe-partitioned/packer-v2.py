...
def segments(raw, name):
    ...
    def walk(start, path, default="metadata"):
        ...
        if len(path)==4 and path[:2]==("execution","rows") and path[3] in ("formation","generations") and value is not None:
            return [(start,end,"formations" if path[3]=="formation" else "generations")],end
        ...
        for key in keys:
            ...
            spans.append((previous,cursor,default))
