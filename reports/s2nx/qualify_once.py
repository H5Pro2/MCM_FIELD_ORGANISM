"""Single neutral NX source qualification, bound before starting unittest."""
import ast
import json
import subprocess
import sys
...
def main():
    ...
    before = b.watched()
    tree = ast.parse((b.ROOT/"tests/test_s2nx_private_source_binding.py").read_text(encoding="utf-8"))
    ...
    command = [sys.executable,"-m","unittest","tests.test_s2nx_private_source_binding","-v","-f"]
    b.publish(out/"preregistration.json",dict(run_id=b.QUAL_ID,command=command,cwd=str(b.ROOT),
        test_ids=["tests.test_s2nx_private_source_binding.SourceTests."+n for n in names],expected_tests=24,
        unittest_calls=1,retry=False,hashes_before=before,environment=b.environment(),
        limits=dict(nx_payloads=0,receptor_calls=0,nj_calls=0,predictions=0,learning_updates=0,error_calculations=0,system_calls=0,
    ...
    result = b.sealed(dict(run_id=b.QUAL_ID,status="S2NX_SOURCE_BINDING_QUALIFIED" if passed else "NOT_QUALIFIED",
        expected_tests=24,passed_tests=24 if passed else None,unittest_calls=1,exit_code=process.returncode,
        hashes_before=before,hashes_after=after,nx_payloads_generated=0,receptor_calls=0,nj_calls=0,
