"""One bounded neutral unittest invocation, following the existing report pattern."""
...
def main():
    ...
    before = b.watched()
    path = b.ROOT / "tests/test_s2np_private_source_binding.py"
    ...
    command = [sys.executable, "-m", "unittest", "tests.test_s2np_private_source_binding", "-v"]
    b.publish(out / "preregistration.json", dict(run_id=b.QUAL_ID, command=command, cwd=str(b.ROOT),
        test_ids=["tests.test_s2np_private_source_binding.SourceBindingTests." + n for n in names],
        expected_tests=16, unittest_calls=1, retry=False, hashes_before=before,
        np_payload_generation_limit=0, neutral_pcm_limit=dict(calls=1, samples=16, bytes=64),
        receptor_calls_limit=0, nj_calls_limit=0, forbidden_imports_checked=True,
        output_limit=b.MAX_OUTPUT_BYTES, metadata_limit=b.MAX_METADATA_BYTES,
    ...
    result = b.sealed(dict(run_id=b.QUAL_ID, status="S2NP_SOURCE_BINDING_QUALIFIED" if passed else "NOT_QUALIFIED",
        exit_code=process.returncode, unittest_calls=1, expected_tests=16, passed_tests=16 if passed else None,
        hashes_before=before, hashes_after=after, np_payload_generation_calls=0, receptor_calls=0, nj_calls=0,
