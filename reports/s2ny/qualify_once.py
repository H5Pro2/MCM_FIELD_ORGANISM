"""Single neutral NY qualification, with inventory and hashes bound first."""
import ast
import json
import subprocess
import sys
...
def main():
    ...
    before=b.watched()
    tree=ast.parse((b.ROOT/"tests/test_s2ny_private_source_binding.py").read_text(encoding="utf-8"))
    ...
    command=[sys.executable,"-m","unittest","tests.test_s2ny_private_source_binding","-v","-f"]
    b.publish(out/"preregistration.json",dict(run_id=b.QUAL_ID,command=command,cwd=str(b.ROOT),
        test_ids=["tests.test_s2ny_private_source_binding.SourceTests."+n for n in names],expected_tests=24,
        unittest_calls=1,retry=False,hashes_before=before,environment=b.environment(),
        limits=dict(ny_payloads=0,neutral_generated_samples=6,neutral_generated_bytes=24,max_live_neutral_bytes=12,
            sin_calls=32,metadata_digest_checks=32768,nx_learning_replays=0,owners_reopened=0,
    ...
    result=b.sealed(dict(run_id=b.QUAL_ID,status="S2NY_SOURCE_BINDING_QUALIFIED" if passed else "NOT_QUALIFIED",
        expected_tests=24,passed_tests=24 if passed else None,unittest_calls=1,exit_code=process.returncode,
        hashes_before=before,hashes_after=after,ny_payloads_generated=0,nx_learning_replays=0,owners_reopened=0,
