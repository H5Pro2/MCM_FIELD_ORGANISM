"""Authorized single NU comparison, one verification, conditional evaluation."""
import json
from tools import _s2nu_private_comparison_run as run

RUN_ID = "s2nu-temporal-comparison-20260909-01"


def main():
    run.c.require(run.MAIN_GATE is False and run.b.MAIN_GATE is False,"GATE_INVALID")
    run.c.require(not (run.b.ROOT/"reports/s2nu"/RUN_ID).exists(),"RUN_ALREADY_EXISTS")
    run.MAIN_GATE = True
    try:
    ...
    assessment = None
    if verification["status"] == "S2NU_COMPARISON_VERIFIED" and verification["evaluation_allowed"] is True:
