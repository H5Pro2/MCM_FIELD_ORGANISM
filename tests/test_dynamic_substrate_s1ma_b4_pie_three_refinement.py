...
class DTS1S1MAB4PIEThreeRefinementTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        with (
            ...
            cls.outputs = runner.run_dts1_b4_pie_three_refinement()
    ...
    def test_executes_three_replicas_twelve_intervals_and_six_fresh_sequences(self) -> None:
        ...
        self.assertEqual(
            runner.S1_MA_TARGET_REPLICA_IDS,
            tuple(output.replica_id for output in self.outputs),
        )

    def test_outputs_are_complete_atomic_b4_pie_v2_records(self) -> None:
    ...
    def test_registry_remains_closed_to_other_new_profiles(self) -> None:
        for replica_id in runner.S1_MA_TARGET_REPLICA_IDS:
            self.assertEqual(
                replica_id,
        ...
        with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
            runner.DTS1OneReplicaRunnerInput(
                runner.S1_KC_RUNNER_INPUT_SCHEMA_ID,
                "B4:P_IE_CAUSAL_TWO_SUBSTEP:r16",
            )
    ...
    def test_triple_and_receipt_are_fail_closed(self) -> None:
        with patch.object(
            ...
            with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
                runner.run_dts1_b4_pie_three_refinement()
        with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
            replace(
                runner.build_dts1_s1ma_implementation_receipt(),
