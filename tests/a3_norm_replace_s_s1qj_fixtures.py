"""Canonical in-memory fixtures for the private S1-QJ compositor."""
...
def norm_spec() -> W7MBaselineSpec:
    adapter = build_w7m_capacity_function_matrix_adapter()
    return next(item for item in adapter.baselines if item.model_id == "norm")


def build_sync_fixture(
    *,
    field: SharedMCMField | None = None,
    norm_prestate: W7NLocalBaselineState | None = None,
    start_tick: int = 0,
    ...
    state = (
        ...
        if norm_prestate is None
        else norm_prestate
    )
    return A3NormFixture(
        current_field,
        distribution,
        interval,
