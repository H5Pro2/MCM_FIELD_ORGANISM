...
class DTS1S1LWB3PINThreeRefinementTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        with (
            ...
            cls.outputs = runner.run_dts1_b3_pin_three_refinement()
    ...
    def test_executes_three_replicas_twenty_four_intervals_and_six_fresh_sequences(self) -> None:
        ...
        self.assertEqual(runner.S1_LW_TARGET_REPLICA_IDS, tuple(output.replica_id for output in self.outputs))

    def test_outputs_are_complete_atomic_b3_pin_v2_records(self) -> None:
        for output in self.outputs:
            self.assertEqual((runner.S1_KF_OUTPUT_SCHEMA_ID, "B3", "P_IN_RELEASE_REUSE"), (output.schema_id, output.model_role, output.profile_block))
    ...
    def test_independent_sequence_terminals_are_bit_identical_by_refinement(self) -> None:
        self.assertEqual(runner.S1_LW_CHECKPOINT_FIELD_DIGESTS, tuple(tuple(row.complete_field_digest for row in output.checkpoints) for output in self.outputs))
        self.assertEqual(runner.S1_LW_CHECKPOINT_PRIVATE_STATE_DIGESTS, tuple(tuple(row.private_state_digest for row in output.checkpoints) for output in self.outputs))
        self.assertEqual(runner.S1_LW_CHECKPOINT_ADAPTER_OUTPUT_DIGESTS, tuple(tuple(row.adapter_output_digest for row in output.checkpoints) for output in self.outputs))
        for rows in (
            runner.S1_LW_CHECKPOINT_FIELD_DIGESTS,
            runner.S1_LW_CHECKPOINT_PRIVATE_STATE_DIGESTS,
            runner.S1_LW_CHECKPOINT_ADAPTER_OUTPUT_DIGESTS,
        ):
    ...
    def test_comparison_and_provenance_digests_are_exact_and_distinct(self) -> None:
        self.assertEqual(runner.S1_LW_TARGET_COMPARISON_DIGESTS, tuple(output.refinement_comparison_digest for output in self.outputs))
        self.assertEqual(runner.S1_LW_TARGET_OUTPUT_DIGESTS, tuple(output.output_digest for output in self.outputs))
    ...
    def test_signed_components_are_zero_without_release_reuse_judgment(self) -> None:
        self.assertEqual(runner.S1_LW_TARGET_COMPONENTS_BY_REFINEMENT, tuple((output.refinement, output.signed_components) for output in self.outputs))
        self.assertTrue(all(value == 0.0 for _, components in runner.S1_LW_TARGET_COMPONENTS_BY_REFINEMENT for value in components))

    def test_receipt_binds_execution_without_case_or_judgment(self) -> None:
        receipt = runner.build_dts1_s1lw_implementation_receipt()
        ...
        self.assertTrue(receipt.independent_sequence_terminals_bit_identical)
    ...
    def test_registry_remains_closed_to_other_new_profiles(self) -> None:
        for replica_id in runner.S1_LW_TARGET_REPLICA_IDS:
        ...
        with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
            runner.DTS1OneReplicaRunnerInput(runner.S1_KC_RUNNER_INPUT_SCHEMA_ID, "B3:P_IN_RELEASE_REUSE:r16")
    ...
    def test_triple_and_receipt_are_fail_closed(self) -> None:
        with patch.object(runner, "run_dts1_one_replica", side_effect=(self.outputs[0], self.outputs[0], self.outputs[2])):
            with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
                runner.run_dts1_b3_pin_three_refinement()
        with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
            replace(runner.build_dts1_s1lw_implementation_receipt(), total_new_interval_calls=25)
