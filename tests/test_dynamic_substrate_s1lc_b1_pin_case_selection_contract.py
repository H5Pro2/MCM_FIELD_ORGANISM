...
from mcm_field_organism.dynamic_substrate_s1lc_b1_pin_case_selection_contract import (
    DTS1S1LCB1PINCaseSelectionContractError,
    S1_LC_DECISION,
    build_dts1_s1lc_b1_pin_case_selection_contract,
)


class DTS1S1LCB1PINCaseSelectionContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1lc_b1_pin_case_selection_contract()

    def test_binds_exact_s1lb_source_and_registered_c04(self) -> None:
        self.assertEqual(64, len(self.contract.source_s1lb_digest))
        self.assertEqual(("C04", "B1", "B1_FIXED_PRERELEASE_ADAPTER", "P_IN_RELEASE_REUSE", 3, 6), self.contract.target_case_record[:6])
    ...
    def test_selects_without_implementation_execution_or_judgment(self) -> None:
        ...
        self.assertEqual(S1_LC_DECISION, self.contract.decision)

    def test_is_deterministic_tamper_evident_and_call_free(self) -> None:
        second = build_dts1_s1lc_b1_pin_case_selection_contract()
        self.assertEqual(self.contract.contract_digest, second.contract_digest)
        with self.assertRaises(DTS1S1LCB1PINCaseSelectionContractError):
            replace(self.contract, maximum_new_interval_calls=25)
        source = inspect.getsource(build_dts1_s1lc_b1_pin_case_selection_contract)
