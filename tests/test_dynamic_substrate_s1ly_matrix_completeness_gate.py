...
from mcm_field_organism.dynamic_substrate_s1ly_matrix_completeness_gate import (
    DTS1S1LYMatrixCompletenessGateError,
    S1_LY_DECISION,
    build_dts1_s1ly_matrix_completeness_gate,
)


class DTS1S1LYMatrixCompletenessGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1ly_matrix_completeness_gate()
    ...
    def test_binds_completed_and_missing_case_counts(self) -> None:
        ...
        self.assertEqual(
            tuple(f"C{index:02d}" for index in range(1, 13)),
            self.contract.completed_case_ids,
        )
        self.assertEqual(
            tuple(f"C{index:02d}" for index in range(13, 25)),
    ...
    def test_binds_digests_for_completed_and_refinements(self) -> None:
        ...
        self.assertEqual(12, len(set(self.contract.completed_case_contract_digests)))
        self.assertEqual(12, len(set(self.contract.completed_case_output_digests)))
    ...
    def test_stays_static_and_deterministic(self) -> None:
        ...
        self.assertEqual(S1_LY_DECISION, self.contract.decision)
        second = build_dts1_s1ly_matrix_completeness_gate()
    ...
    def test_tamper_closed_and_no_runtime_invocation(self) -> None:
        with self.assertRaises(DTS1S1LYMatrixCompletenessGateError):
            replace(self.contract, matrix_composition_authorized=True)
        source = inspect.getsource(build_dts1_s1ly_matrix_completeness_gate)
