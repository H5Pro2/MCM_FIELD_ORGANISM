...
from mcm_field_organism.dynamic_substrate_s1mn_b4_pin_case_output_contract import (
    DTS1S1MNB4PINCaseOutputContractError,
...
class DTS1S1MNB4PINCaseOutputContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1mn_b4_pin_case_output_contract()

    def test_binds_exact_sources_and_c16_case(self) -> None:
        self.assertEqual(
            (64, 64),
            (
                len(self.contract.source_s1ml_digest),
                len(self.contract.source_s1mm_receipt_digest),
            ),
        )
        self.assertEqual(S1_MN_SOURCE_S1ML_DIGEST, self.contract.source_s1ml_digest)
        self.assertEqual(S1_MN_SOURCE_S1MM_DIGEST, self.contract.source_s1mm_receipt_digest)
        self.assertEqual(
    ...
    def test_binds_three_refinements_and_zero_primary_components(self) -> None:
        ...
        self.assertEqual(S1_MM_TARGET_REPLICA_IDS, payload["replica_ids"])
        self.assertEqual(
            (2, 4, 8),
            tuple(int(value.rsplit("r", 1)[1]) for value in payload["replica_ids"]),
        )
        self.assertEqual(S1_MM_TARGET_COMPONENTS_BY_REFINEMENT, payload["components_by_refinement"])
    ...
    def test_binds_two_directed_zero_residual_blocks(self) -> None:
        ...
        self.assertEqual(dict(S1_MN_REFINEMENT_RESIDUALS), residuals)
        self.assertEqual(
            (2, 12),
            (
    ...
    def test_executes_nothing_and_authorizes_only_matrix_gate(self) -> None:
        ...
        self.assertEqual(S1_MN_DECISION, self.contract.decision)

    def test_case_payload_digest_and_chain_are_deterministic(self) -> None:
        self.assertEqual(S1_MN_CASE_OUTPUT_DIGEST, self.contract.case_output_digest)
        self.assertEqual(tuple(_case_payload().items()), self.contract.case_payload)
        second = build_dts1_s1mn_b4_pin_case_output_contract()
        self.assertEqual(self.contract.contract_digest, second.contract_digest)
        self.assertEqual(S1_MN_CASE_OUTPUT_DIGEST, second.case_output_digest)

    def test_tamper_closed_and_call_free(self) -> None:
        with self.assertRaises(DTS1S1MNB4PINCaseOutputContractError):
            replace(self.contract, release_reuse_judgment_present=True)
        with self.assertRaises(DTS1S1MNB4PINCaseOutputContractError):
            replace(self.contract, memory_capability_claim_present=True)
        s1ml = build_dts1_s1ml_b4_pin_case_selection_contract()
        s1mm = build_dts1_s1mm_implementation_receipt()
        self.assertEqual(S1_MN_SOURCE_S1ML_DIGEST, s1ml.contract_digest)
        self.assertEqual(S1_MN_SOURCE_S1MM_DIGEST, s1mm.receipt_digest)
        source = inspect.getsource(build_dts1_s1mn_b4_pin_case_output_contract)
        for forbidden in ("run_dts1_b4_pin_three_refinement", "run_dts1_one_replica", "materialize_", "advance_"):
