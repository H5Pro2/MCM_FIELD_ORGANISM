"""Neutral source-only NV tests; corpus generation is blocked in every body."""
...
def bundle():
    # Synthetic hash tokens from metadata, never hashes of generated NV PCM.
    ...
    se = b.sealed(dict(schema="s2nv.source-seal.v1",run_id="neutral",status="S2NV_SOURCES_PRESEALED",
        execution_digest=ex["execution_digest"],evaluation_digest=ev["evaluation_digest"],
        hashes_before=hashes,hashes_after=hashes,attempted_sources=20,completed_sources=20,generated_pcm_bytes=384000,
        max_live_payloads=1,raw_payloads_persisted=0,main_gate_after=False,prefix_groups=b.prefix_groups(),
        collisions=b.common.collision_groups(sources),receptor_calls=0,nj_calls=0,prediction_calls=0,error_calls=0,
...
class SourceTests(unittest.TestCase):
    ...
    def code(self,expected,fn,*args):
        with self.assertRaises(b.S2NVError) as caught:
            fn(*args)
    ...
    def test_01_literal_sources(self):
        ...
        self.assertEqual(len({s.payload()["source_id"] for s in sources}),20)
        self.assertEqual(b.ROWS,(((0,4),(0,5),(0,6),(0,7),(0,8)),((0,4),(0,5),(0,6),(0,5),(0,4)),
                               ((0,4),(0,5),(0,6),(0,6),(0,6)),((0,4),(0,5),(0,6),(1,6),(1,6))))
        self.assertEqual(sources[-1].payload()["source_id"],"nv-s04-w04")
    ...
    def test_04_phase_and_null_position(self):
        group = dict(seed="nv-neutral-phase",partials=[dict(frequency_ratio=[101,1],amplitude_ratio=[0,1]),
            dict(frequency_ratio=[203,1],amplitude_ratio=[1,10])])
        actual = b.prepare_groups([group])[0]
        for p,value in enumerate(actual):
            u = int.from_bytes(hashlib.sha256(f"nv-neutral-phase:{p}".encode("ascii")).digest()[:4],"little")
    ...
    def test_06_single_float32_rounding_after_gain(self):
        ...
        with patch.object(b,"group_value",side_effect=inputs),patch.object(b.struct,"pack_into",wraps=pack) as calls:
            data = b.render(NEUTRAL,(7,10),(0,1,2))
        self.assertEqual(calls.call_count,3)
        expected = [(7.0/10.0)*x for x in inputs]
    ...
    def test_08_complete_neutral_bundle(self):
        ...
        self.assertEqual(result["windows"],20)
        self.assertEqual(result["forecast_sites_bound_not_executed"],12)
    ...
    def test_10_sites_and_prefix_manipulations(self):
        ex,ev,se,h = bundle()
        self.assertEqual(len(ex["forecast_sites"]),12)
        ex["forecast_sites"][0]["available_source_ids"].append("nv-s01-w02")
        refresh(ex,ev,se)
        self.code("FORECAST_SITE_INVALID",v.verify_bundle,ex,ev,se,h)
        ex,ev,se,h = bundle()
        se["prefix_groups"][0][0] = "nv-s01-w03"
        reseal(se,"seal_digest")
    ...
    def test_12_evaluation_roots_and_six_conditions(self):
        ex,ev,se,h = bundle()
        self.assertNotIn("categories",ex)
        self.assertNotIn("criteria",ex)
        self.assertEqual([(c["site_id"],c["operator"]) for c in ev["criteria"]],
            [("p01","LT"),("p02","LT"),("p03","LT"),("p05","GT"),("p08","GT"),("p11","GT")])
    ...
    def test_13_resource_limits(self):
        ...
        self.code("PAYLOAD_SIZE_INVALID",b.render,NEUTRAL,(3,10),tuple(range(4801)))
        ex["oversize"] = "x"*65536
        refresh(ex,ev,se)
    ...
    def test_14_immutability_profile_and_digest(self):
        ...
        self.assertEqual(s.recipe()["group"]["seed"],"s2nv-pcm-001")
        ex,ev,se,h = bundle()
    ...
    def test_16_fail_closed_and_exclusions(self):
        ...
        with patch.object(b,"group_value",return_value=float("inf")):
            self.code("PCM_DOMAIN_INVALID",b.render,NEUTRAL,(3,10),(0,))
        ex,ev,se,h = bundle()
