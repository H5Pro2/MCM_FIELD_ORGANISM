...
class DTS1S1LAB2PIKThreeRefinementTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        with patch.object(runner, "materialize_dts1_common_interval", counted_materializer), patch.object(runner, "advance_dts1_private_baseline", counted_adapter), patch.object(runner, "_build_fresh_state", counted_fresh):
            cls.outputs = runner.run_dts1_b2_pik_three_refinement()
    ...
    def test_executes_three_replicas_twenty_four_intervals_and_six_fresh_sequences(self) -> None:
        ...
        self.assertEqual(runner.S1_LA_TARGET_REPLICA_IDS, tuple(output.replica_id for output in self.outputs))

    def test_outputs_are_complete_atomic_b2_pik_v2_records(self) -> None:
        for output in self.outputs:
            self.assertEqual((runner.S1_KF_OUTPUT_SCHEMA_ID, "B2", "P_IK_INTERFERENCE"), (output.schema_id, output.model_role, output.profile_block))
    ...
    def test_sequence_terminals_differ_and_are_refinement_identical(self) -> None:
        for output in self.outputs:
            self.assertEqual(runner.S1_LA_TERMINAL_FIELD_DIGESTS, tuple(row.complete_field_digest for row in output.checkpoints))
            self.assertEqual(runner.S1_LA_TERMINAL_PRIVATE_STATE_DIGESTS, tuple(row.private_state_digest for row in output.checkpoints))
            self.assertEqual(runner.S1_LA_TERMINAL_ADAPTER_OUTPUT_DIGESTS, tuple(row.adapter_output_digest for row in output.checkpoints))
        self.assertTrue(all(len(set(values)) == 2 for values in (runner.S1_LA_TERMINAL_FIELD_DIGESTS, runner.S1_LA_TERMINAL_PRIVATE_STATE_DIGESTS, runner.S1_LA_TERMINAL_ADAPTER_OUTPUT_DIGESTS)))

    def test_comparison_and_provenance_digests_are_exact(self) -> None:
        self.assertEqual((runner.S1_LA_TARGET_COMPARISON_DIGEST,) * 3, tuple(output.refinement_comparison_digest for output in self.outputs))
        self.assertEqual(runner.S1_LA_TARGET_OUTPUT_DIGESTS, tuple(output.output_digest for output in self.outputs))
        self.assertEqual(3, len({output.output_digest for output in self.outputs}))

    def test_signed_components_are_nonzero_and_refinement_identical(self) -> None:
        self.assertEqual((runner.S1_LA_TARGET_COMPONENTS,) * 3, tuple(output.signed_components for output in self.outputs))
        self.assertTrue(all(value != 0.0 for value in runner.S1_LA_TARGET_COMPONENTS))

    def test_receipt_binds_execution_without_case_or_judgment(self) -> None:
        receipt = runner.build_dts1_s1la_implementation_receipt()
        ...
        self.assertTrue(receipt.sequence_terminal_digests_distinct)
        self.assertTrue(receipt.terminal_digest_pairs_bit_identical_across_refinements)
    ...
    def test_registry_remains_closed_to_other_new_profiles(self) -> None:
        for replica_id in runner.S1_LA_TARGET_REPLICA_IDS:
    ...
    def test_triple_and_receipt_are_fail_closed(self) -> None:
        with patch.object(runner, "run_dts1_one_replica", side_effect=(self.outputs[0], self.outputs[0], self.outputs[2])):
            with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
                runner.run_dts1_b2_pik_three_refinement()
        with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
            replace(runner.build_dts1_s1la_implementation_receipt(), total_new_interval_calls=25)
