...
class DTS1S1MIB4PIKThreeRefinementTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        with (
            ...
            cls.outputs = runner.run_dts1_b4_pik_three_refinement()
    ...
    def test_executes_three_replicas_twenty_four_intervals_and_six_fresh_sequences(self) -> None:
        ...
        self.assertEqual(
            runner.S1_MI_TARGET_REPLICA_IDS,
    ...
    def test_outputs_are_complete_atomic_b4_pik_v2_records(self) -> None:
        for output in self.outputs:
            self.assertEqual(
                (runner.S1_KF_OUTPUT_SCHEMA_ID, "B4", "P_IK_INTERFERENCE"),
    ...
    def test_comparison_and_provenance_digests_are_exact_and_distinct(self) -> None:
        self.assertEqual(
            runner.S1_MI_TARGET_COMPARISON_DIGESTS,
            tuple(output.refinement_comparison_digest for output in self.outputs),
        )
        self.assertEqual(
            runner.S1_MI_TARGET_OUTPUT_DIGESTS,
    ...
    def test_signed_components_are_bound_by_refinement_without_judgment(self) -> None:
        self.assertEqual(
            runner.S1_MI_TARGET_COMPONENTS_BY_REFINEMENT,
            tuple((output.refinement, output.signed_components) for output in self.outputs),
        )
        self.assertFalse(
            all(
                value == 0.0
                for _, components in runner.S1_MI_TARGET_COMPONENTS_BY_REFINEMENT
                for value in components
            )
        )

    def test_receipt_binds_execution_without_case_judgment_or_claims(self) -> None:
        receipt = runner.build_dts1_s1mi_implementation_receipt()
        self.assertEqual(
            (3, 2, 4, 8, 24),
            (
    ...
    def test_registry_remains_closed_to_other_new_profiles(self) -> None:
        for replica_id in runner.S1_MI_TARGET_REPLICA_IDS:
            self.assertEqual(
                replica_id,
    ...
    def test_triple_and_receipt_are_fail_closed(self) -> None:
        with patch.object(
            ...
            with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
                runner.run_dts1_b4_pik_three_refinement()
        with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
            replace(
                runner.build_dts1_s1mi_implementation_receipt(),
