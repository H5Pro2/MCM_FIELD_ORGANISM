...
class DTS1S1MEB4PIHThreeRefinementTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        with (
            ...
            cls.outputs = runner.run_dts1_b4_pih_three_refinement()
    ...
    def test_executes_three_replicas_nine_intervals_and_three_fresh_starts(self) -> None:
        ...
        self.assertEqual(
            runner.S1_ME_TARGET_REPLICA_IDS,
    ...
    def test_outputs_are_complete_atomic_b4_pih_records(self) -> None:
        for output in self.outputs:
            self.assertEqual(
                (runner.S1_KF_OUTPUT_SCHEMA_ID, "B4", "P_IH_ATTENUATION"),
    ...
    def test_provenance_and_comparison_digests_are_distinct_and_exact(self) -> None:
        self.assertEqual(
            runner.S1_ME_TARGET_OUTPUT_DIGESTS,
            tuple(output.output_digest for output in self.outputs),
        )
        self.assertEqual(
            runner.S1_ME_TARGET_COMPARISON_DIGESTS,
            tuple(output.refinement_comparison_digest for output in self.outputs),
        )
        self.assertEqual(
            (3, 3),
            (
                len(set(runner.S1_ME_TARGET_OUTPUT_DIGESTS)),
                len(set(runner.S1_ME_TARGET_COMPARISON_DIGESTS)),
            ),
        )

    def test_components_are_bound_by_refinement_and_not_all_zero(self) -> None:
        self.assertEqual(
            runner.S1_ME_TARGET_COMPONENTS_BY_REFINEMENT,
            tuple((output.refinement, output.signed_components) for output in self.outputs),
        )
        self.assertFalse(
            all(
                value == 0.0
                for _, values in runner.S1_ME_TARGET_COMPONENTS_BY_REFINEMENT
                for value in values
            )
        )

    def test_receipt_binds_execution_without_case_or_judgment_or_claims(self) -> None:
        receipt = runner.build_dts1_s1me_implementation_receipt()
        self.assertEqual(
            (3, 1, 3, 3, 9),
            (
        ...
        self.assertEqual(runner.S1_ME_TARGET_OUTPUT_DIGESTS, tuple(receipt.target_output_digests))
        ...
        self.assertTrue(receipt.signed_components_nonzero_present)
    ...
    def test_registry_remains_closed_to_other_new_profiles(self) -> None:
        for replica_id in runner.S1_ME_TARGET_REPLICA_IDS:
            self.assertEqual(
                replica_id,
    ...
    def test_triple_and_receipt_are_fail_closed(self) -> None:
        with patch.object(
            ...
            with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
                runner.run_dts1_b4_pih_three_refinement()
        with self.assertRaises(runner.DTS1OneReplicaOrchestratorError):
            replace(
                runner.build_dts1_s1me_implementation_receipt(),
                total_new_interval_calls=10,
            )
