...
from mcm_field_organism.dynamic_substrate_s1mc_matrix_completeness_gate import (
    DTS1S1MCMatrixCompletenessGateError,
    S1_MC_DECISION,
    build_dts1_s1mc_matrix_completeness_gate,
)


class DTS1S1MCMatrixCompletenessGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1mc_matrix_completeness_gate()
    ...
    def test_binds_completed_and_missing_case_counts(self) -> None:
        ...
        self.assertEqual(
            tuple(f"C{index:02d}" for index in range(1, 14)),
            self.contract.completed_case_ids,
        )
        self.assertEqual(
            tuple(f"C{index:02d}" for index in range(14, 25)),
    ...
    def test_binds_digests_for_completed_and_refinements(self) -> None:
        ...
        self.assertEqual(13, len(set(self.contract.completed_case_contract_digests)))
        self.assertEqual(13, len(set(self.contract.completed_case_output_digests)))
    ...
    def test_blocks_matrix_composition_and_restricts_next_case(self) -> None:
        ...
        self.assertTrue(self.contract.c14_selection_authorized_next_stage)
        self.assertEqual(
            (
                "C14",
                "B4",
    ...
    def test_stays_static_and_deterministic(self) -> None:
        ...
        self.assertEqual(S1_MC_DECISION, self.contract.decision)
        second = build_dts1_s1mc_matrix_completeness_gate()
    ...
    def test_tamper_closed_and_no_runtime_invocation(self) -> None:
        with self.assertRaises(DTS1S1MCMatrixCompletenessGateError):
            replace(self.contract, memory_capability_claim_present=True)
        with self.assertRaises(DTS1S1MCMatrixCompletenessGateError):
            replace(self.contract, matrix_composition_authorized=True)
        source = inspect.getsource(build_dts1_s1mc_matrix_completeness_gate)
