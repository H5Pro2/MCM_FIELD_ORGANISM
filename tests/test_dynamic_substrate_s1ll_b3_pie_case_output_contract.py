...
from mcm_field_organism.dynamic_substrate_s1ll_b3_pie_case_output_contract import (
    DTS1S1LLB3PIECaseOutputContractError,
    S1_LL_DECISION,
    build_dts1_s1ll_b3_pie_case_output_contract,
)


class DTS1S1LLB3PIECaseOutputContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1ll_b3_pie_case_output_contract()

    def test_binds_exact_gate_output_sources_and_c09(self) -> None:
        self.assertEqual((64, 64), (len(self.contract.source_s1li_digest), len(self.contract.source_s1lk_digest)))
        self.assertEqual(("C09", "B3", "B3_F3_LOCAL_LEAKY", "P_IE_CAUSAL_TWO_SUBSTEP", 2, 8), self.contract.source_s1jx_case_record[:6])
    ...
    def test_binds_three_refinements_and_distinct_digests(self) -> None:
        ...
        self.assertEqual((3, 3, 3), (self.contract.replica_count, self.contract.distinct_provenance_digest_count, self.contract.comparison_digest_count))

    def test_binds_checkpoints_and_independent_sequence_identity(self) -> None:
        self.assertEqual((2, 4), (self.contract.sequence_count_per_refinement, self.contract.checkpoint_count_per_refinement))
    ...
    def test_primary_output_is_r4_with_eight_zero_components(self) -> None:
        payload = dict(self.contract.case_payload)
        self.assertEqual((4, 8), (self.contract.primary_refinement, self.contract.component_count_per_refinement))
    ...
    def test_binds_two_complete_directed_residual_blocks(self) -> None:
        ...
        self.assertEqual((2, 16), (self.contract.residual_block_count, self.contract.residual_component_count))
    ...
    def test_executes_nothing_and_authorizes_only_c10_selection(self) -> None:
        self.assertEqual((0, 0), (self.contract.new_replicas_executed, self.contract.new_interval_calls_executed))
        self.assertTrue(self.contract.c10_selection_authorized_next_stage)
        self.assertEqual(S1_LL_DECISION, self.contract.decision)

    def test_is_deterministic_tamper_evident_and_call_free(self) -> None:
        second = build_dts1_s1ll_b3_pie_case_output_contract()
        self.assertEqual(self.contract.contract_digest, second.contract_digest)
        with self.assertRaises(DTS1S1LLB3PIECaseOutputContractError):
            replace(self.contract, all_refinement_residuals_zero=False)
        source = inspect.getsource(build_dts1_s1ll_b3_pie_case_output_contract)
