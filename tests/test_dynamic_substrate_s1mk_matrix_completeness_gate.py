...
from mcm_field_organism.dynamic_substrate_s1mk_matrix_completeness_gate import (
    DTS1S1MKMatrixCompletenessGateError,
    S1_MK_DECISION,
    build_dts1_s1mk_matrix_completeness_gate,
)


class DTS1S1MKMatrixCompletenessGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1mk_matrix_completeness_gate()
    ...
    def test_binds_completed_and_missing_case_counts(self) -> None:
        ...
        self.assertEqual(
            tuple(f"C{index:02d}" for index in range(1, 16)),
            self.contract.completed_case_ids,
        )
        self.assertEqual(
            tuple(f"C{index:02d}" for index in range(16, 25)),
    ...
    def test_binds_digests_for_completed_cases_and_refinements(self) -> None:
        ...
        self.assertEqual(15, len(set(self.contract.completed_case_contract_digests)))
        self.assertEqual(15, len(set(self.contract.completed_case_output_digests)))
    ...
    def test_blocks_matrix_composition_and_restricts_next_case(self) -> None:
        ...
        self.assertTrue(self.contract.c16_selection_authorized_next_stage)
        self.assertEqual(
            (
                "C16",
                "B4",
    ...
    def test_stays_static_and_deterministic(self) -> None:
        ...
        self.assertEqual(S1_MK_DECISION, self.contract.decision)
        second = build_dts1_s1mk_matrix_completeness_gate()
    ...
    def test_tamper_closed_and_no_runtime_invocation(self) -> None:
        with self.assertRaises(DTS1S1MKMatrixCompletenessGateError):
            replace(self.contract, memory_capability_claim_present=True)
        with self.assertRaises(DTS1S1MKMatrixCompletenessGateError):
            replace(self.contract, matrix_composition_authorized=True)
        source = inspect.getsource(build_dts1_s1mk_matrix_completeness_gate)
