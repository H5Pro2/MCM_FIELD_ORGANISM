...
from mcm_field_organism.dynamic_substrate_s1mt_b5_pih_case_selection_contract import (
    DTS1S1MTB5PIHCaseSelectionContractError,
    S1_MT_DECISION,
    build_dts1_s1mt_b5_pih_case_selection_contract,
)


class DTS1S1MTB5PIHCaseSelectionContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1mt_b5_pih_case_selection_contract()

    def test_binds_exact_s1ms_source_and_registered_c18(self) -> None:
        self.assertEqual(64, len(self.contract.source_s1ms_digest))
        self.assertEqual(
            (
                "C18",
                "B5",
    ...
    def test_binds_complete_b5_fresh_state_and_full_arm(self) -> None:
        fresh = self.contract.corrected_fresh_state_record
        self.assertEqual(("B5", "TWO_NODE_OPEN_LINE", ("node-a", "node-b")), fresh[:3])
        substrate = dict(dict(fresh[5])["substrate"])
        arm = dict(substrate["arm"])
        self.assertEqual((("node-a", 0.5), ("node-b", 0.5)), substrate["masses"])
        self.assertEqual(self.contract.b5_edge_inventory_digest, substrate["edge_inventory_digest"])
        self.assertEqual("mcm.s1jt.b5.full", arm["arm_id"])
        self.assertEqual(
            (1.0, 0.5, 1.0, 1.0),
            (
    ...
    def test_binds_embedded_m_state_and_b5_configuration(self) -> None:
        ...
        self.assertEqual(
            self.contract.b5_configuration_digest,
            private["B5_configuration_digest"],
        )
