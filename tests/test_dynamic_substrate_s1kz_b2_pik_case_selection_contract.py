...
from mcm_field_organism.dynamic_substrate_s1kz_b2_pik_case_selection_contract import (
    DTS1S1KZB2PIKCaseSelectionContractError,
    S1_KZ_DECISION,
    build_dts1_s1kz_b2_pik_case_selection_contract,
)


class DTS1S1KZB2PIKCaseSelectionContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1kz_b2_pik_case_selection_contract()

    def test_binds_exact_s1ky_source_and_registered_c07(self) -> None:
        self.assertEqual(64, len(self.contract.source_s1ky_digest))
        self.assertEqual(("C07", "B2", "B2_S2_LINEAR_INTEGRATOR", "P_IK_INTERFERENCE", 3, 6), self.contract.target_case_record[:6])
    ...
    def test_selects_without_implementation_execution_or_judgment(self) -> None:
        ...
        self.assertEqual(S1_KZ_DECISION, self.contract.decision)

    def test_is_deterministic_tamper_evident_and_call_free(self) -> None:
        second = build_dts1_s1kz_b2_pik_case_selection_contract()
        self.assertEqual(self.contract.contract_digest, second.contract_digest)
        with self.assertRaises(DTS1S1KZB2PIKCaseSelectionContractError):
            replace(self.contract, maximum_new_interval_calls=25)
        source = inspect.getsource(build_dts1_s1kz_b2_pik_case_selection_contract)
