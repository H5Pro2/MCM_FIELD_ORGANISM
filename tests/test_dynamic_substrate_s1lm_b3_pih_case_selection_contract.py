...
from mcm_field_organism.dynamic_substrate_s1lm_b3_pih_case_selection_contract import (
    DTS1S1LMB3PIHCaseSelectionContractError,
    S1_LM_DECISION,
    build_dts1_s1lm_b3_pih_case_selection_contract,
)


class DTS1S1LMB3PIHCaseSelectionContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.contract = build_dts1_s1lm_b3_pih_case_selection_contract()

    def test_binds_exact_s1ll_source_and_registered_c10(self) -> None:
        self.assertEqual(64, len(self.contract.source_s1ll_digest))
        self.assertEqual(("C10", "B3", "B3_F3_LOCAL_LEAKY"), self.contract.target_case_record[:3])
    ...
    def test_binds_embedded_m_state_and_single_pih_carry(self) -> None:
        ...
        self.assertIn("complete-field-M-state-and-bound-arm-carry-only-between-the-three-ordered-intervals", joined)

    def test_binds_dual_digest_roles_and_b3_nonsummation_rule(self) -> None:
        ...
        self.assertIn("may-differ-across-r2-r4-r8", rules)

    def test_binds_nine_call_budget_without_retry(self) -> None:
        self.assertEqual((3, 1, 3, 3, 9), (self.contract.target_replica_count, self.contract.sequences_per_target_replica, self.contract.intervals_per_sequence, self.contract.intervals_per_target_replica, self.contract.maximum_new_interval_calls))
    ...
    def test_selects_without_implementation_execution_or_judgment(self) -> None:
        ...
        self.assertEqual(S1_LM_DECISION, self.contract.decision)

    def test_is_deterministic_tamper_evident_and_call_free(self) -> None:
        second = build_dts1_s1lm_b3_pih_case_selection_contract()
        self.assertEqual(self.contract.contract_digest, second.contract_digest)
        with self.assertRaises(DTS1S1LMB3PIHCaseSelectionContractError):
            replace(self.contract, maximum_new_interval_calls=10)
        source = inspect.getsource(build_dts1_s1lm_b3_pih_case_selection_contract)
