    ...
    with zipfile.ZipFile(path, "r") as z:
        ...
        for name,kind,size,sha,parts in files:
            ...
            for n in sorted({p[0] for p in parts}):
                ...
                require(sum(map(len,cache.values()))+length<=MAX_MEMBER,"UNPACK_MEMORY_LIMIT")
            ...
            for n,o,count in parts:
                restored.extend(memoryview(cache[n])[o:o+count]); expanded[objects[n][1]]+=count
            ...
            def merged(ranges):
                out=[]
                for length,k in ranges:
                    if not length:continue
                    if out and out[-1][1]==k:out[-1][0]+=length
                    else:out.append([length,k])
                return out
            expected=merged((end-start,k) for start,end,k in segments(restored,name))
            actual=merged((count,objects[n][1]) for n,offset,count in parts)
            require(actual==expected,"CLASS_CHANGED")
