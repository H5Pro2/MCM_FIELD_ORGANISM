"""Independent literal NZ source reconstruction, without PCM regeneration."""
...
def verify_bundle(ex,ev,seal,hashes,freezes):
    ...
    b.require(set(ex)=={"schema","contract_sha256","sources","source_order","environment","profiles",
        "source_hashes","generator","freeze_import","forecast_sites","noise_contract","prediction_contract",
        "budgets","authorizations","execution_digest"},"EXECUTION_FORM_INVALID")
    b.require(ex["schema"]=="s2nz.source-execution-plan.v1" and ev["schema"]=="s2nz.diagnostic-evaluation-plan.v1"
        and seal["schema"]=="s2nz.source-seal.v1","SCHEMA_INVALID")
    ...
    b.require(ex["authorizations"]=={key:False for key in b.FORBIDDEN_CALLS},"AUTHORIZATION_INVALID")
    gains=((128,320,368,380,383),)*2+((128,320,464,572,653),)*2+((128,320,464,464,464),)*2
    order=[f"nz-s{s:02d}-w{k:02d}" for s in range(1,7) for k in range(5)]
    ...
    for n,row in enumerate(ex["sources"]):
        ...
        fs=(730,2190,6570) if group==0 else (1010,3030,9090)
        seed=None if s%2==0 else f"s2nz-noise-{s//2+1:03d}"
        recipe=dict(schema="s2nz.pcm-window-recipe.v1",sample_rate=48000,sample_count=4800,
            group=dict(seed=f"s2nz-pcm-{group+1:03d}",partials=[dict(frequency_ratio=[f,1],amplitude_ratio=[a,20])
                for f,a in zip(fs,(4,2,1),strict=True)]),gain_ratio=[gains[s][k],1024],
            noise=dict(seed=seed,window=k if seed else None,amplitude_ratio=[1 if seed else 0,1024]),
            synthesis_time="float(j)/48000.0; j=0..4799",rounding="group-sum;gain;optional-noise-add;single-f32le")
        expected=dict(source_id=order[n],ordinal=n+1,stream_id=f"s{s+1:02d}",window_ordinal=k,format="PCM_F32LE",
            channels=1,clock_id="audio.sample",window_start_sample=n*4800,window_end_sample=(n+1)*4800,
            nj_snapshot_index=n*10,pcm_byte_count=19200,recipe=recipe,recipe_digest=b.digest(recipe))
        b.check(row,"source_digest")
        b.require(b.old.sha(row.get("pcm_sha256")),"PAYLOAD_HASH_INVALID")
    ...
    b.require(ex["noise_contract"]==b.noise_contract(),"NOISE_CONTRACT_INVALID")
    b.require(ex["prediction_contract"]==b.prediction_contract(),"PREDICTOR_BOUNDARY_INVALID")
    ...
    for rows in recipes.values():
        b.require(len({r["pcm_sha256"] for r in rows})==1 and len({r["source_digest"] for r in rows})==len(rows),"EXACT_COPY_BINDING_INVALID")
    ...
    b.require(seal["status"]=="S2NZ_SOURCES_PRESEALED" and
        all(type(seal[k]) is int and seal[k]==30 for k in ("attempted_sources","completed_sources"))
        and seal["generated_pcm_bytes"]==576000 and seal["max_live_payloads"]==1 and seal["noise_hashes"]==72000
    ...
    return dict(windows=30,streams=6,freeze_payloads_bound=2,forecast_sites_bound=18,local_sites_bound=12,
        diagnostic_focus_N=4,emitted_D_separate=True,practical_criteria=0,collisions=collisions,
...
def verify_once(out):
    ...
    try:
        ...
        b.require(seal["execution_file_sha256"]==before[names[0]] and
            seal["evaluation_file_sha256"]==before[names[1]],"FILE_BINDING_INVALID")
        ...
        report=dict(run_id=out.name,status="S2NZ_PRESEAL_VERIFIED",execution_digest=ex["execution_digest"],
