"""Private append-only recorder for the bounded S2-IG run."""
...
def _write_exclusive(path: Path, payload: object, maximum: int) -> tuple[str, int]:
    ...
    if len(encoded) > maximum or len(encoded) > fixtures.MAX_INDIVIDUAL_ARTIFACT_BYTES:
        raise S2IGRecordingError("IG-E008", "registered resource limit exceeded")
    ...
    try:
        ...
        raise S2IGRecordingError("IG-E010", "exclusive publication failed") from error
...
class ExecutionPlan:
    ...
    def build(
        ...
        if (
            ...
            raise S2IGRecordingError("IG-E003", "execution plan binding is invalid")
        payload = {
            ...
            "maximum_success_bytes": registry.maximum_success_bytes,
            "maximum_failure_bytes": registry.maximum_failure_bytes,
        }
        return cls(
            run_id,
            owner_id,
            ...
            registry.maximum_success_bytes,
            registry.maximum_failure_bytes,
...
class AppendOnlyRunRecorder:
    """Record one registry-ordered run without invoking project logic."""
    ...
    def current_row(self) -> fixtures.OperationRow:
        if not 1 <= self.next_operation_index <= len(self.registry.rows):
            raise S2IGRecordingError("IG-E002", "operation registry exhausted")
    ...
    def _append_event(
        ...
        if len(encoded) > fixtures.MAX_EVENT_BYTES:
            raise S2IGRecordingError("IG-E008", "event resource limit exceeded")
        ...
        if self.byte_count > self.plan.maximum_failure_bytes:
            raise S2IGRecordingError("IG-E008", "run resource limit exceeded")
        return event_digest
    ...
    def start(
        ...
        *,
        ...
        self.pending_start = (operation_id, start_digest)
        return start_digest
    ...
    def finish(self, operation_id: str, artifact: dict[str, object]) -> str:
        ...
        if self.pending_start is None or self.pending_start[0] != operation_id:
            raise S2IGRecordingError("IG-E007", "result has no matching start")
        envelope = {
            "schema": RECORDER_SCHEMA,
        ...
        artifact_digest, artifact_bytes = _write_exclusive(
            self.run_directory / row.target_path,
            envelope,
    ...
    def fail(self, error_code: str, failed_operation_id: str) -> None:
        if self.state in TERMINAL_STATES:
            raise S2IGRecordingError("IG-E012", "terminal run cannot fail again")
        if error_code not in fixtures.ERROR_CODES:
            error_code = "IG-E009"
        row = self.current_row()
        if row.operation_id != failed_operation_id:
            raise S2IGRecordingError("IG-E002", "failed operation differs")
        ...
        closures = (
            ("ie-err-001", "failure/receipt.json", "FAILING"),
            ("ie-err-002", "terminal/failure/NOT_EVALUABLE", "NOT_EVALUABLE"),
        )
        for offset, (operation_id, target, success_state) in enumerate(closures, 1):
            failure_row = fixtures.OperationRow(
                fixtures.SUCCESS_OPERATION_COUNT + offset,
                operation_id,
                "FAILURE",
                "FAILURE_CLOSE",
                row.history_id,
                row.case_id,
                (row.operation_id,) if offset == 1 else ("ie-err-001",),
                "run_owner",
                "S2IGTerminalFinding",
                target,
                1_024,
            ...
            payload = {
                ...
                "prior_failure_artifact_digest": self.result_digests.get("ie-err-001"),
            }
            artifact_digest, artifact_bytes = _write_exclusive(
        ...
        if self.event_count > fixtures.MAX_FAILURE_EVENT_COUNT:
            raise S2IGRecordingError("IG-E008", "failure event limit exceeded")
