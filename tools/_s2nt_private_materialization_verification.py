"""Offline NT raw-to-half verification; no receptor, NJ or source-pair calls."""
import hashlib
import json
import math
import struct
...
from tools import _s2nt_private_receptor_materialization as m
...
def inspect_record(out):
    ...
    m.require(result["schema"] == "s2nt.receptor-nj-materialization.v1" and result["run_id"] == out.name == m.RUN_ID
    ...
    m.require(type(result["states"]) is list and len(result["states"]) <= 14,"STATE_COUNT_INVALID")
    checked = 0
    ...
    m.require(c["completed_sources"] == len(result["states"]) <= c["nj_returns"] <= c["nj_attempts"]
        <= c["analyze_returns"] <= c["analyze_attempts"] <= c["payloads_validated"] <= c["generation_attempts"] <= 14,"PROGRESS_INVALID")
    if complete:
        m.require(all(c[k] == 14 for k in ("generation_attempts","payloads_validated","analyze_attempts","analyze_returns",
            "nj_attempts","nj_returns","completed_sources")) and c["raw_value_count"] == c["half_value_count"] == checked == 672,"COUNTERS_INVALID")
    ...
    return dict(run_id=m.RUN_ID,status="S2NT_MATERIALIZATION_VALID" if complete else "TECHNICAL_FAILURE_RECORDED",
