...
CAUCHY_PAIRS = ((20, 40), (40, 80), (80, 160))
SHARD_AUDIT_ID = (
...
class CauchyConvergenceShardMergeError(ValueError):
    pass


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise CauchyConvergenceShardMergeError(message)


def _validate_group(group: dict, alpha: float) -> None:
    _require(
        float(group.get("alpha_per_amplitude_second", -1.0)) == alpha,
        "group alpha does not match its shard",
    )
    ...
    _require([item.get("scheme") for item in results] == list(SCHEMES),
...
def _validate_shard(payload: dict) -> float:
    ...
    _require(float(payload.get("shard_value", -1.0)) == alpha,
             "shard value and alpha axis differ")
    ...
    _require(len({group.get("start_layer_digest") for group in groups}) == 1,
             "start layer state differs across shard durations")
    _require(len({group.get("start_snapshot_digest") for group in groups}) == 1,
             "start snapshot state differs across shard durations")
...
def merge_payloads(payloads: list[dict]) -> dict:
    _require(len(payloads) == len(ALPHA_AXIS), "exactly three shards are required")
    by_alpha = {}
    ...
    for alpha in ALPHA_AXIS[1:]:
        for field in COMMON_FIELDS:
            _require(by_alpha[alpha].get(field) == reference.get(field),
    ...
    merged["groups"] = [
        ...
        for alpha in ALPHA_AXIS
        for group in sorted(
            by_alpha[alpha]["groups"], key=lambda item: item["duration_ticks"]
        )
    ]
    return merged
...
def merge_shard_files(paths=SHARD_PATHS, output: Path = OUTPUT) -> Path:
    ...
    _require(len(paths) == len(ALPHA_AXIS), "exactly three shard paths are required")
    ...
    for path in paths:
        ...
        try:
            ...
            raise CauchyConvergenceShardMergeError(f"invalid shard: {path}") from exc
