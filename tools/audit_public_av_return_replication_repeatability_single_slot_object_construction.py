...
from audit_public_av_return_replication_repeatability_single_slot_object_reservation import (  # noqa: E402
    MEDIA_PATH,
)
...
from mcm_field_organism.public_av_return_replication_repeatability_single_slot_object_construction import prepare_public_av_return_replication_repeatability_single_slot_object_construction, public_av_return_replication_repeatability_single_slot_object_construction_to_jsonable  # noqa: E402
from mcm_field_organism.public_av_return_replication_repeatability_single_slot_object_reservation import reserve_public_av_return_replication_repeatability_single_slot_objects  # noqa: E402
...
def build_construction(repeat_index: int):
    ...
    return prepare_public_av_return_replication_repeatability_single_slot_object_construction(reservation)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit one locked single-slot object construction contract.")
    parser.add_argument("--repeat-index", type=int, choices=(1, 2, 3), default=1)
    args = parser.parse_args(argv)
    construction = build_construction(args.repeat_index)
    print(json.dumps(
        public_av_return_replication_repeatability_single_slot_object_construction_to_jsonable(construction),
        indent=2, sort_keys=True,
    ))
    return 0 if construction.constructor_identities_declared else 1
