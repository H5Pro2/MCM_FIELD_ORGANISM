"""Receptor-free NW preseal; prediction formulae are metadata, not executable."""
import ast
...
MAIN_GATE = False
QUAL_ID = "s2nw-source-binding-qualification-20260909-01"
RUN_ID = "s2nw-source-preseal-20260909-01"
QUAL_DIR = ROOT / "reports/s2nw" / QUAL_ID
CONTRACT = "docs/S2NW_STATISCHER_PLAN_GELERNTE_AUDITIVE_VERLAUFSVORHERSAGE.md"
PINS = {
    CONTRACT: "246f43d3f713aa638b586a0deee04c18066077db77115639c3a8c6e977887b98",
...
OWN = ("tools/_s2nw_private_source_binding.py", "tools/_s2nw_private_preseal_verification.py",
       "tests/test_s2nw_private_source_binding.py", "reports/s2nw/qualify_once.py",
       "reports/s2nw/preseal_once.py", "reports/s2nw/QUALIFIKATIONSBINDUNG.md")
MAX_METADATA_BYTES = 65536
MAX_OUTPUT_BYTES = 2097152
GROUPS = (((530, 1590, 4770), "s2nw-pcm-001"), ((710, 2130, 6390), "s2nw-pcm-002"),
          ((890, 2670, 8010), "s2nw-pcm-003"))
# Stream 0 is l01. All gain denominators are 64; no learned coefficient here.
ROWS = (((0, 16), (0, 32), (0, 40), (0, 44), (0, 46), (0, 47)),
        ((1, 20), (1, 28), (1, 32), (1, 34), (1, 35)),
        ((1, 20), (1, 28), (1, 32), (1, 28), (1, 26)),
        ((1, 20), (1, 28), (1, 32), (1, 32), (1, 32)),
        ((1, 20), (1, 28), (1, 32), (2, 32), (2, 32)))


class S2NWError(ValueError):
...
class WindowSpec:
    ...
    def __post_init__(self):
        require(type(self.stream) is int and 0 <= self.stream <= 4
                and type(self.window) is int and 0 <= self.window < (6 if self.stream == 0 else 5), "WINDOW_SPEC_INVALID")

    def recipe(self):
        group, numerator = ROWS[self.stream][self.window]
        frequencies, seed = GROUPS[group]
        return dict(schema="s2nw.pcm-window-recipe.v1", sample_rate=48000, sample_count=4800,
            group=dict(seed=seed, partials=[dict(frequency_ratio=[f, 1], amplitude_ratio=[a, 20])
                for f, a in zip(frequencies, (4, 2, 1), strict=True)]),
            gain_ratio=[numerator, 64], synthesis_time="float(j)/48000.0; j=0..4799",
            rounding="binary64-group-sum-then-gain-then-single-f32le")

    def payload(self):
        n = self.window if self.stream == 0 else 6 + (self.stream - 1) * 5 + self.window
        stream_id = "l01" if self.stream == 0 else f"s{self.stream:02d}"
        start = n * 4800
        recipe = self.recipe()
        return dict(source_id=f"nw-{stream_id}-w{self.window:02d}",
...
def specs():
    return tuple(WindowSpec(s, k) for s in range(5) for k in range(6 if s == 0 else 5))
...
def render(group, gain_ratio, indices):
    ...
    require(type(gain_ratio) in (tuple, list) and len(gain_ratio) == 2
            and all(type(x) is int for x in gain_ratio) and gain_ratio[1] == 64
            and 0 <= gain_ratio[0] <= 64, "GAIN_FORM_INVALID")
    gain = float(gain_ratio[0]) / 64.0
...
def forecast_sites():
    ...
    for stream in range(5):
        sid = "l01" if stream == 0 else f"s{stream:02d}"
        for origin in range(1, 5 if stream == 0 else 4):
            sites.append(dict(site_id=f"t{origin:02d}" if stream == 0 else f"p{(stream-1)*3+origin:02d}",
                stream_id=sid, origin=origin, target=origin+1,
                phase="TRAIN" if stream == 0 else "FROZEN",
                available_source_ids=[f"nw-{sid}-w{j:02d}" for j in range(origin+1)],
                delta_source_ids=[f"nw-{sid}-w{origin-1:02d}", f"nw-{sid}-w{origin:02d}"],
                persist_source_id=f"nw-{sid}-w{origin:02d}", target_source_id=f"nw-{sid}-w{origin+1:02d}",
                update_after_target=stream == 0))
    return sites


def learning_schedule():
    return dict(training_stream="l01", update_sites=["t01","t02","t03","t04"],
        update_after_observed_targets=[f"nw-l01-w{k:02d}" for k in (2,3,4,5)],
        ...
        reset_prefix_each_stream=True, reset_learning_between_test_streams=False,
        same_frozen_state_all_test_sites=True, test_updates_allowed=False,
...
def budgets():
    return dict(windows=26,generated_samples=124800,generated_pcm_bytes=499200,max_live_payloads=1,max_live_pcm_bytes=19200,
        future_analyses=26,future_nj=26,future_values_per_scale=1248,training_sites=4,test_sites=12,forecast_sites=16,
        predictions_per_implementation=48,learned_subtractions_per_implementation=768,
        learned_multiplications_per_implementation=768,learned_additions_per_implementation=768,
        linear_subtractions_per_implementation=768,linear_additions_per_implementation=768,persist_copies_per_implementation=768,
        update_differences_per_implementation=384,update_products_per_implementation=384,update_additions_per_implementation=384,
        updates_per_implementation=4,max_update_divisions_per_implementation=4,
        error_terms_per_implementation=2304,mae_sums_per_implementation=48,gains_per_implementation=32,
        both_error_terms=4608,both_mae_sums=96,both_gains=64,
        offline_halvings=1248,offline_prediction_subtractions=3072,offline_prediction_multiplications=1536,
        offline_prediction_additions=3072,offline_persist_copies=1536,
        offline_update_differences=768,offline_update_products=768,offline_update_additions=768,
        offline_updates=8,offline_max_update_divisions=8,offline_error_terms=4608,offline_sums=96,offline_gains=64,
        outcome_classifications=32,training_outcome_classifications=8,strict_criteria=9,
        max_learning_state_bytes=4096,max_prefix_vectors=3,max_metadata_bytes=65536,max_output_bytes=2097152,
        max_verification_bytes=262144,max_evaluation_bytes=262144)


def execution_plan(sources, env, hashes, generator):
    require(type(sources) is list and len(sources) == 26, "SOURCE_COUNT_INVALID")
    ...
    return sealed(dict(schema="s2nw.source-execution-plan.v1",contract_sha256=PINS[CONTRACT],sources=sources,
...
def evaluation_plan(execution):
    criteria = [dict(check_id=f"o{2*(k-1)+j:02d}",site_id=f"p{k:02d}",left="MAE_LEARNED_DELTA",
        operator="LT",right="MAE_"+arm,role="PRIMARY")
        for k in (1,2,3) for j,arm in ((1,"PERSIST"),(2,"LINEAR"))]
    criteria += [dict(check_id=f"o{n:02d}",site_id=site,left="MAE_LEARNED_DELTA",
        operator="GT",right="MAE_PERSIST",role="SEPARATE_SWITCH_LOSS")
        for n,site in ((7,"p05"),(8,"p08"),(9,"p11"))]
    return sealed(dict(schema="s2nw.evaluation-plan.v1",execution_digest=execution["execution_digest"],
        contract_sha256=PINS[CONTRACT],categories=dict(l01="TRAINING_DECAYING_INCREMENT",s01="CONTINUATION",
            s02="REVERSAL",s03="STANDSTILL_AFTER_RISE",s04="UNEXPECTED_GROUP_CHANGE"),
        criteria=criteria,positive_learning_requires=dict(observed_updates=4,noninitial_frozen_state=True,Sxx_strictly_positive=True),
        reporting=dict(per_test_stream_per_baseline_denominator=3,per_phase_denominator=1,
            ...
            common_prefix_is_independent_replication=False),
...
def prefix_groups():
    return [[f"nw-s{s:02d}-w{k:02d}" for s in range(1,5)] for k in (0,1,2)]
...
def qualification(hashes):
    ...
    require(q.get("result_digest")==digest({k:v for k,v in q.items() if k!="result_digest"})
        and q.get("run_id")==QUAL_ID and q.get("status")=="S2NW_SOURCE_BINDING_QUALIFIED"
        and q.get("passed_tests")==20 and q.get("unittest_calls")==1 and q.get("exit_code")==0
...
def preseal_once(run_id):
    ...
    try:
        ...
        seal = sealed(dict(schema="s2nw.source-seal.v1",run_id=run_id,status="S2NW_SOURCES_PRESEALED",
            ...
            generated_pcm_bytes=499200,max_live_payloads=1,raw_payloads_persisted=0,prefix_groups=prefix_groups(),
