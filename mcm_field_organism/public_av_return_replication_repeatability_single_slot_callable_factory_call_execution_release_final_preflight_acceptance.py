"""Locked acceptance of one callable-factory call execution release final preflight."""
...
from .public_av_return_replication_repeatability_single_slot_callable_factory_call_execution_release_final_preflight import (
...
ID = (
    ...
    "callable-factory-call-execution-release-final-preflight-acceptance.v1"
)


class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptanceError(
    ValueError
):
    pass
...
class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptance:
    acceptance_id: str
    selected_repeat_index: int
    final_preflight: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflight
    positive_final_preflight_accepted: bool
    exactly_one_final_lock_candidate_accepted: bool
    final_lock_candidate_unconsumed_accepted: bool
    ...
    def __post_init__(self) -> None:
        error = PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptanceError
        preflight = self.final_preflight
        source = (
            preflight.execution_order_acceptance.execution_order.execution_preflight_acceptance
        ...
        if candidate.role != "callable_factory" or candidate.executed or not candidate.one_time_future_step:
            raise error("final lock candidate must remain unconsumed")
        if gate.role != "gate_factory" or gate.executed or not gate.one_time_future_step:
            raise error("gate step must remain untouched")
        required = (
            self.positive_final_preflight_accepted,
            self.exactly_one_final_lock_candidate_accepted,
            self.final_lock_candidate_unconsumed_accepted,
        ...
        if not all(required) or any(forbidden):
            raise error("final preflight acceptance remains fully non-executable")


def accept_public_av_return_replication_repeatability_single_slot_callable_factory_call_execution_release_final_preflight(
    preflight: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflight,
) -> PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptance:
    error = PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptanceError
    if not isinstance(preflight, PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflight):
        raise error("wrong final preflight type")
    required = (
        preflight.positive_execution_order_acceptance_bound,
        preflight.exactly_one_final_lock_candidate_bound,
        preflight.final_lock_candidate_unconsumed,
        preflight.callable_identity_binding_accepted,
        preflight.gate_step_unselected,
        preflight.gate_step_untouched,
        preflight.gate_step_still_unexecuted,
        preflight.selected_slot_still_fresh,
        preflight.final_preflight_complete,
    )
    forbidden = (
    ...
    if not all(required) or any(forbidden):
        raise error("one complete locked final preflight is required")
    index = preflight.selected_repeat_index
    return PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptance(
        acceptance_id=f"{ID}.repeat-{index}.v1",
        selected_repeat_index=index,
        final_preflight=preflight,
        positive_final_preflight_accepted=True,
        exactly_one_final_lock_candidate_accepted=True,
        final_lock_candidate_unconsumed_accepted=True,
...
def execute_public_av_return_replication_repeatability_single_slot_accepted_callable_factory_call_execution_release_final_preflight(
    acceptance: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptance,
) -> None:
    del acceptance
    raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptanceError(
        "factory call execution release remains locked"
    )


def public_av_return_replication_repeatability_single_slot_callable_factory_call_execution_release_final_preflight_acceptance_to_jsonable(
    acceptance: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionReleaseFinalPreflightAcceptance,
) -> dict[str, Any]:
