"""Static S1-LF selection contract for the finite B2/P_IN case."""
...
class DTS1S1LFB2PINCaseSelectionContractError(ValueError):
    """Raised when the finite S1-LF case selection is weakened."""


S1_LF_CONTRACT_ID = "dynamic-substrate.b2-pin-case-selection.s1lf.v1"
S1_LF_SOURCE_S1LE_DIGEST = (
    "9c6a97a47a9fda8a14590aca0c67b4fd109f67b0824a2bd22b49c2bc8522b812"
)
S1_LF_CASE_ID = "C08"
S1_LF_SEQUENCE_KEYS = ("P_IN_RECOVERY_ON", "P_IN_RECOVERY_OFF")
S1_LF_TARGET_REPLICA_IDS = (
    "B2:P_IN_RELEASE_REUSE:r2",
    "B2:P_IN_RELEASE_REUSE:r4",
    "B2:P_IN_RELEASE_REUSE:r8",
)
S1_LF_TARGET_CASE_RECORD = next(
    row for row in S1_JX_CASE_RECORDS if row[0] == S1_LF_CASE_ID
)
S1_LF_TARGET_REPLICA_RECORDS = tuple(
    row for row in S1_JX_REPLICA_RECORDS if row[0] in S1_LF_TARGET_REPLICA_IDS
)
S1_LF_SEQUENCE_RECORDS = tuple(
    row for row in S1_JX_SEQUENCE_RECORDS if row[0] in S1_LF_SEQUENCE_KEYS
)
S1_LF_FRESH_STATE_RECORD = next(
    row for row in S1_JZ_FRESH_STATE_RECORDS
    if row[0] == "B2" and row[1] == "THREE_NODE_OPEN_LINE"
)
S1_LF_FRESH_FIELD_DIGEST = (
    "55278f3e39a856f4846a45f65a0e4289b486b1a3bb5c32deb5eede0828e3c2d8"
)
S1_LF_FRESH_PRIVATE_STATE_DIGEST = (
    "c835a011e26d20129f6a44f31f8147dd09f96686cd3427bbf89020ce23c59827"
)
S1_LF_FRESH_START_AND_CARRY_RULES = (
    "r2-r4-and-r8-each-use-two-independent-corrected-B2-fresh-states-one-per-sequence",
    "P_IN_RECOVERY_ON-and-P_IN_RECOVERY_OFF-each-start-once-from-bit-identical-fresh-field-and-complete-zero-L-state",
...
S1_LF_OUTPUT_ACCEPTANCE_RULES = (
    ...
    "all-three-replicas-must-pass-before-C08-may-be-composed-in-a-later-stage",
)
S1_LF_SELECTION_RATIONALE = (
    "C08-is-the-stateful-B2-counterpart-to-technically-complete-B1-C04",
    "B2-receives-the-same-model-neutral-recovery-on-versus-recovery-off-history-and-checkpoint-plan-as-B1",
    "selection-does-not-predict-sign-threshold-release-reuse-baseline-closure-or-candidate-outcome",
)
S1_LF_FORBIDDEN_SCOPE = (
    "no-runner-initializer-adapter-or-output-implementation",
    "no-replica-sequence-interval-retry-or-repeat-execution",
    "no-C08-output-or-24-case-matrix-publication",
...
class DTS1S1LFB2PINCaseSelectionContract:
    contract_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.contract_id != S1_LF_CONTRACT_ID
            or self.source_s1le_digest != S1_LF_SOURCE_S1LE_DIGEST
            or self.target_case_record != S1_LF_TARGET_CASE_RECORD
            or self.target_replica_ids != S1_LF_TARGET_REPLICA_IDS
            or self.target_replica_records != S1_LF_TARGET_REPLICA_RECORDS
            or self.sequence_records != S1_LF_SEQUENCE_RECORDS
            or self.corrected_fresh_state_record != S1_LF_FRESH_STATE_RECORD
            or self.fresh_field_digest != S1_LF_FRESH_FIELD_DIGEST
            or self.fresh_private_state_digest != S1_LF_FRESH_PRIVATE_STATE_DIGEST
            ...
            or self.fresh_start_and_carry_rules != S1_LF_FRESH_START_AND_CARRY_RULES
            or self.execution_budget != S1_LF_EXECUTION_BUDGET
            or self.output_acceptance_rules != S1_LF_OUTPUT_ACCEPTANCE_RULES
            or self.selection_rationale != S1_LF_SELECTION_RATIONALE
            or self.forbidden_scope != S1_LF_FORBIDDEN_SCOPE
            ...
            or self.decision != S1_LF_DECISION
            or self.contract_digest != _digest(payload)
        ):
            raise DTS1S1LFB2PINCaseSelectionContractError(
                "S1-LF weakened the finite B2/P_IN case selection"
            )


def build_dts1_s1lf_b2_pin_case_selection_contract(
) -> DTS1S1LFB2PINCaseSelectionContract:
    """Select and bind C08 without implementing or executing its replicas."""

    source = build_dts1_s1le_b1_pin_case_output_contract()
    if (
        source.contract_digest != S1_LF_SOURCE_S1LE_DIGEST
        or len(S1_LF_TARGET_REPLICA_RECORDS) != 3
        or tuple(row[0] for row in S1_LF_TARGET_REPLICA_RECORDS)
        != S1_LF_TARGET_REPLICA_IDS
        or tuple(row[0] for row in S1_LF_SEQUENCE_RECORDS) != S1_LF_SEQUENCE_KEYS
        or tuple(row[3] for row in S1_LF_SEQUENCE_RECORDS) != (4, 4)
        or tuple(row[5] for row in S1_LF_SEQUENCE_RECORDS) != ((4,), (4,))
        or S1_LF_FRESH_STATE_RECORD[6] != S1_LF_FRESH_FIELD_DIGEST
        or S1_LF_FRESH_STATE_RECORD[8] != S1_LF_FRESH_PRIVATE_STATE_DIGEST
    ):
        raise DTS1S1LFB2PINCaseSelectionContractError(
            "registered C08 evidence or corrected B2 fresh state differs"
        )
    values = {
        "contract_id": S1_LF_CONTRACT_ID,
        "source_s1le_digest": source.contract_digest,
        "target_case_record": S1_LF_TARGET_CASE_RECORD,
        "target_replica_ids": S1_LF_TARGET_REPLICA_IDS,
        "target_replica_records": S1_LF_TARGET_REPLICA_RECORDS,
        "sequence_records": S1_LF_SEQUENCE_RECORDS,
        "corrected_fresh_state_record": S1_LF_FRESH_STATE_RECORD,
        "fresh_field_digest": S1_LF_FRESH_FIELD_DIGEST,
        "fresh_private_state_digest": S1_LF_FRESH_PRIVATE_STATE_DIGEST,
        ...
        "fresh_start_and_carry_rules": S1_LF_FRESH_START_AND_CARRY_RULES,
        "execution_budget": S1_LF_EXECUTION_BUDGET,
        "output_acceptance_rules": S1_LF_OUTPUT_ACCEPTANCE_RULES,
        "selection_rationale": S1_LF_SELECTION_RATIONALE,
        "forbidden_scope": S1_LF_FORBIDDEN_SCOPE,
        "target_replica_count": 3,
        ...
        "decision": S1_LF_DECISION,
    }
    return DTS1S1LFB2PINCaseSelectionContract(
