"""Locked execution preflight for one single-slot factory order."""
...
from .public_av_return_replication_repeatability_single_slot_factory_order_acceptance import (
    PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderAcceptance,
)


SINGLE_SLOT_FACTORY_ORDER_EXECUTION_PREFLIGHT_ID = (
    "public.av.nasa-earthrise.return-replication."
    "repeatability-single-slot-factory-order-execution-preflight.v1"
)


class PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(ValueError):
    pass


@dataclass(frozen=True)
class PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflight:
    preflight_id: str
    ...
    source_id: str
    positive_factory_order_acceptance_bound: bool
    two_ordered_execution_candidates_bound: bool
    callable_factory_execution_candidate_first: bool
    gate_factory_execution_candidate_second: bool
    execution_candidate_order_fixed: bool
    factory_order_identities_bound: bool
    selected_slot_still_fresh: bool
    factory_order_execution_preflight_complete: bool
    ...
    def __post_init__(self) -> None:
        if self.selected_repeat_index not in (1, 2, 3):
            raise PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(
                "selected repeat index must be one of 1, 2, 3"
            )
        suffix = f".repeat-{self.selected_repeat_index}.v1"
        indexed_ids = (
            self.preflight_id, self.factory_order_acceptance_id, self.factory_order_id,
        ...
        if not all(identifier.endswith(suffix) for identifier in indexed_ids):
            raise PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(
                "factory order execution preflight identities must match selected repeat index"
            )
        expected_candidates = (
        ...
        if tuple(self.ordered_factory_execution_candidate_ids) != expected_candidates:
            raise PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(
                "ordered execution candidates must match callable then gate factory orders"
            )
        if self.future_callable_factory_order_id == self.future_gate_factory_order_id:
            raise PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(
                "factory order identities must remain unique"
            )
        ...
        required = (
            self.positive_factory_order_acceptance_bound,
            self.two_ordered_execution_candidates_bound,
            self.callable_factory_execution_candidate_first,
            self.gate_factory_execution_candidate_second,
            self.execution_candidate_order_fixed,
            self.factory_order_identities_bound,
            self.selected_slot_still_fresh,
            self.factory_order_execution_preflight_complete,
        )
        forbidden = (
        ...
        if not all(required) or any(forbidden):
            raise PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(
                "factory order execution preflight cannot store references, call factories, create objects, bind, decode, feed receptors, start runs, or release claims"
            )
        object.__setattr__(
            self,
...
def preflight_public_av_return_replication_repeatability_single_slot_factory_order_execution(
    ...
    if not isinstance(acceptance, PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderAcceptance):
        raise PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(
            "single-slot factory order acceptance has the wrong type"
        )
    required = (
        acceptance.positive_factory_order_accepted,
        acceptance.two_factory_order_identities_accepted,
        acceptance.factory_order_identities_unique,
        ...
        acceptance.factory_order_acceptance_complete,
    )
    forbidden = (
    ...
    if not all(required) or any(forbidden):
        raise PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(
            "one complete fresh reference-free factory order acceptance is required"
        )

    index = acceptance.selected_repeat_index
    suffix = f"repeat-{index}.v1"
    return PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflight(
        preflight_id=f"{SINGLE_SLOT_FACTORY_ORDER_EXECUTION_PREFLIGHT_ID}.{suffix}",
        selected_repeat_index=index,
        factory_order_acceptance_id=acceptance.acceptance_id,
        ...
        ordered_factory_execution_candidate_ids=(
            acceptance.future_callable_factory_order_id,
            acceptance.future_gate_factory_order_id,
        ),
...
def execute_public_av_return_replication_repeatability_single_slot_factory_order_preflight(
    preflight: PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflight,
) -> None:
    del preflight
    raise PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflightError(
        "factory order execution is not released by the locked execution preflight"
    )


def public_av_return_replication_repeatability_single_slot_factory_order_execution_preflight_to_jsonable(
    preflight: PublicAVReturnReplicationRepeatabilitySingleSlotFactoryOrderExecutionPreflight,
) -> dict[str, Any]:
