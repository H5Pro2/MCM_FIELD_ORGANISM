"""Locked order for one callable-factory call."""
...
from .public_av_return_replication_repeatability_single_slot_callable_factory_call_preflight_acceptance import (
    PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptance,
)
...
SINGLE_SLOT_CALLABLE_FACTORY_CALL_ORDER_ID = (
    "public.av.nasa-earthrise.return-replication."
    "repeatability-single-slot-callable-factory-call-order.v1"
)


class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrderError(ValueError):
    pass


@dataclass(frozen=True)
class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrder:
    call_order_id: str
    selected_repeat_index: int
    call_preflight_acceptance_id: str
    call_preflight_id: str
    ...
    future_callable_call_step: PublicAVReturnReplicationRepeatabilitySingleSlotFactoryExecutionStep
    untouched_gate_factory_step: PublicAVReturnReplicationRepeatabilitySingleSlotFactoryExecutionStep
    future_callable_call_step_id: str
    ...
    source_id: str
    positive_call_preflight_acceptance_bound: bool
    exactly_one_future_callable_call_derived: bool
    callable_call_one_time: bool
    callable_call_unexecuted: bool
    ...
    callable_factory_call_order_complete: bool
    ...
    def __post_init__(self) -> None:
        error = PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrderError
        ...
        indexed_ids = (
            self.call_order_id, self.call_preflight_acceptance_id, self.call_preflight_id,
            self.callable_factory_execution_order_acceptance_id,
            ...
            self.factory_execution_order_id, self.future_callable_call_step_id,
        ...
        if not all(identifier.endswith(suffix) for identifier in indexed_ids):
            raise error("callable factory call order identities must match selected repeat index")
        step = self.future_callable_call_step
        gate_step = self.untouched_gate_factory_step
        if not (
            step.step_index == 1 and step.role == "callable_factory"
            and step.step_id == self.future_callable_call_step_id
            ...
            raise error("future call step must remain the one-time unexecuted callable factory step")
        if not (
        ...
        required = (
            self.positive_call_preflight_acceptance_bound,
            self.exactly_one_future_callable_call_derived,
            self.callable_call_one_time,
            self.callable_call_unexecuted,
            ...
            self.callable_factory_call_order_complete,
        )
        forbidden = (
        ...
        if not all(required) or any(forbidden):
            raise error("callable factory call order remains fully non-executable")
        object.__setattr__(self, "other_slots_unselected", tuple(self.other_slots_unselected))


def order_public_av_return_replication_repeatability_single_slot_callable_factory_call(
    acceptance: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptance,
) -> PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrder:
    error = PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrderError
    if not isinstance(acceptance, PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptance):
        raise error("single-slot callable factory call preflight acceptance has the wrong type")
    required = (
        acceptance.positive_call_preflight_accepted,
        acceptance.exactly_one_callable_call_candidate_accepted,
        ...
        acceptance.callable_factory_call_preflight_acceptance_complete,
    )
    forbidden = (
    ...
    if not all(required) or any(forbidden):
        raise error("one complete fresh callable factory call preflight acceptance is required")
    carried_fields = (
        "selected_repeat_index", "call_preflight_id",
        "callable_factory_execution_order_acceptance_id", "callable_factory_execution_order_id",
    ...
    return PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrder(
        call_order_id=f"{SINGLE_SLOT_CALLABLE_FACTORY_CALL_ORDER_ID}.repeat-{index}.v1",
        call_preflight_acceptance_id=acceptance.acceptance_id,
        future_callable_call_step=acceptance.accepted_call_candidate_step,
        future_callable_call_step_id=acceptance.accepted_call_candidate_step_id,
        positive_call_preflight_acceptance_bound=True,
        exactly_one_future_callable_call_derived=True,
        callable_call_one_time=True,
        callable_call_unexecuted=True,
        ...
        callable_factory_call_order_complete=True,
        **carried,
    )


def execute_public_av_return_replication_repeatability_single_slot_callable_factory_call_order(
    order: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrder,
) -> None:
    del order
    raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrderError(
        "factory call is not released by the locked callable factory call order"
    )


def public_av_return_replication_repeatability_single_slot_callable_factory_call_order_to_jsonable(
    order: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrder,
) -> dict[str, Any]:
    return asdict(order)
