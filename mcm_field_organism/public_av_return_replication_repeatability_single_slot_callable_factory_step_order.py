"""Locked one-time order for the first callable-factory step."""
...
from .public_av_return_replication_repeatability_single_slot_first_factory_step_acceptance import (
    PublicAVReturnReplicationRepeatabilitySingleSlotFirstFactoryStepAcceptance,
)


SINGLE_SLOT_CALLABLE_FACTORY_STEP_ORDER_ID = (
    "public.av.nasa-earthrise.return-replication."
    "repeatability-single-slot-callable-factory-step-order.v1"
)


class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrderError(ValueError):
    pass


@dataclass(frozen=True)
class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrder:
    order_id: str
    ...
    source_id: str
    positive_first_step_acceptance_bound: bool
    exactly_one_callable_factory_order_derived: bool
    callable_factory_order_one_time: bool
    callable_factory_order_unexecuted: bool
    ...
    callable_factory_step_order_complete: bool
    ...
    def __post_init__(self) -> None:
        if self.selected_repeat_index not in (1, 2, 3):
            raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrderError(
                "selected repeat index must be one of 1, 2, 3"
            )
        suffix = f".repeat-{self.selected_repeat_index}.v1"
        indexed_ids = (
            self.order_id, self.first_factory_step_acceptance_id,
            self.first_factory_step_preflight_id, self.factory_execution_order_acceptance_id,
            self.factory_execution_order_id, self.selected_callable_factory_step_id,
        ...
        if not all(identifier.endswith(suffix) for identifier in indexed_ids):
            raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrderError(
                "callable factory step order identities must match selected repeat index"
            )
        ...
        if not (
            ...
            raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrderError(
                "callable factory step order must target the first unexecuted callable step"
            )
        if not (
        ...
        required = (
            self.positive_first_step_acceptance_bound,
            self.exactly_one_callable_factory_order_derived,
            self.callable_factory_order_one_time,
            self.callable_factory_order_unexecuted,
            ...
            self.callable_factory_step_order_complete,
        )
        forbidden = (
        ...
        if not all(required) or any(forbidden):
            raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrderError(
                "callable factory step order remains fully non-executable"
            )
        object.__setattr__(self, "other_slots_unselected", tuple(self.other_slots_unselected))


def order_public_av_return_replication_repeatability_single_slot_callable_factory_step(
    ...
    if not isinstance(acceptance, PublicAVReturnReplicationRepeatabilitySingleSlotFirstFactoryStepAcceptance):
        raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrderError(
            "single-slot first factory step acceptance has the wrong type"
        )
    required = (
        acceptance.positive_first_step_preflight_accepted,
        acceptance.callable_factory_step_selection_accepted,
        acceptance.callable_factory_step_unconsumed_accepted,
        ...
        acceptance.first_factory_step_acceptance_complete,
    )
    forbidden = (
    ...
    if not all(required) or any(forbidden):
        raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrderError(
            "one complete fresh non-executable first step acceptance is required"
        )
    index = acceptance.selected_repeat_index
    return PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrder(
        order_id=f"{SINGLE_SLOT_CALLABLE_FACTORY_STEP_ORDER_ID}.repeat-{index}.v1",
        selected_repeat_index=index,
        first_factory_step_acceptance_id=acceptance.acceptance_id,
        first_factory_step_preflight_id=acceptance.first_factory_step_preflight_id,
        factory_execution_order_acceptance_id=acceptance.factory_execution_order_acceptance_id,
        factory_execution_order_id=acceptance.factory_execution_order_id,
        selected_factory_step=acceptance.selected_factory_step,
        untouched_gate_factory_step=acceptance.untouched_gate_factory_step,
        selected_callable_factory_step_id=acceptance.selected_callable_factory_step_id,
        ...
        callable_factory_identity_id=acceptance.callable_factory_identity_id,
        callable_constructor_identity_id=acceptance.callable_constructor_identity_id,
        future_callable_object_id=acceptance.future_callable_object_id,
        gate_factory_identity_id=acceptance.gate_factory_identity_id,
        gate_constructor_identity_id=acceptance.gate_constructor_identity_id,
...
def execute_public_av_return_replication_repeatability_single_slot_callable_factory_step_order(
    order: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrder,
) -> None:
    del order
    raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrderError(
        "factory execution is not released by the locked callable factory step order"
    )


def public_av_return_replication_repeatability_single_slot_callable_factory_step_order_to_jsonable(
    order: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryStepOrder,
) -> dict[str, Any]:
    return asdict(order)
