"""Locked acceptance of one callable-factory call preflight."""
...
from .public_av_return_replication_repeatability_single_slot_callable_factory_call_preflight import (
...
SINGLE_SLOT_CALLABLE_FACTORY_CALL_PREFLIGHT_ACCEPTANCE_ID = (
    "public.av.nasa-earthrise.return-replication."
    "repeatability-single-slot-callable-factory-call-preflight-acceptance.v1"
)


class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptanceError(ValueError):
    pass


@dataclass(frozen=True)
class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptance:
    acceptance_id: str
    ...
    accepted_call_candidate_step: PublicAVReturnReplicationRepeatabilitySingleSlotFactoryExecutionStep
    untouched_gate_factory_step: PublicAVReturnReplicationRepeatabilitySingleSlotFactoryExecutionStep
    accepted_call_candidate_step_id: str
    ...
    source_id: str
    positive_call_preflight_accepted: bool
    exactly_one_callable_call_candidate_accepted: bool
    ...
    callable_factory_call_preflight_acceptance_complete: bool
    ...
    def __post_init__(self) -> None:
        error = PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptanceError
        ...
        indexed_ids = (
            self.acceptance_id, self.call_preflight_id,
            ...
            self.factory_execution_order_id, self.accepted_call_candidate_step_id,
        ...
        if not all(identifier.endswith(suffix) for identifier in indexed_ids):
            raise error("call preflight acceptance identities must match selected repeat index")
        candidate = self.accepted_call_candidate_step
        gate_step = self.untouched_gate_factory_step
        if not (
            candidate.step_index == 1 and candidate.role == "callable_factory"
            and candidate.step_id == self.accepted_call_candidate_step_id
            ...
            raise error("accepted call candidate must remain the unconsumed callable factory step")
        if not (
        ...
        required = (
            self.positive_call_preflight_accepted,
            self.exactly_one_callable_call_candidate_accepted,
            ...
            self.callable_factory_call_preflight_acceptance_complete,
        )
        forbidden = (
        ...
        if not all(required) or any(forbidden):
            raise error("call preflight acceptance remains fully non-executable")
        object.__setattr__(self, "other_slots_unselected", tuple(self.other_slots_unselected))


def accept_public_av_return_replication_repeatability_single_slot_callable_factory_call_preflight(
    ...
    error = PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptanceError
    if not isinstance(preflight, PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflight):
        raise error("single-slot callable factory call preflight has the wrong type")
    required = (
        preflight.positive_execution_order_acceptance_bound,
        preflight.exactly_one_callable_call_candidate_bound,
        ...
        preflight.callable_factory_call_preflight_complete,
    )
    forbidden = (
    ...
    if not all(required) or any(forbidden):
        raise error("one complete fresh callable factory call preflight is required")
    carried_fields = (
        "selected_repeat_index", "callable_factory_execution_order_acceptance_id",
        "callable_factory_execution_order_id", "execution_preflight_acceptance_id",
        "execution_preflight_id", "callable_factory_step_order_acceptance_id",
    ...
    return PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptance(
        acceptance_id=f"{SINGLE_SLOT_CALLABLE_FACTORY_CALL_PREFLIGHT_ACCEPTANCE_ID}.repeat-{index}.v1",
        call_preflight_id=preflight.preflight_id,
        accepted_call_candidate_step=preflight.call_candidate_step,
        accepted_call_candidate_step_id=preflight.call_candidate_step_id,
        positive_call_preflight_accepted=True,
        exactly_one_callable_call_candidate_accepted=True,
        ...
        callable_factory_call_preflight_acceptance_complete=True,
        **carried,
    )


def execute_public_av_return_replication_repeatability_single_slot_accepted_callable_factory_call_preflight(
    acceptance: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptance,
) -> None:
    del acceptance
    raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptanceError(
        "factory call is not released by the locked call preflight acceptance"
    )


def public_av_return_replication_repeatability_single_slot_callable_factory_call_preflight_acceptance_to_jsonable(
    acceptance: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallPreflightAcceptance,
) -> dict[str, Any]:
