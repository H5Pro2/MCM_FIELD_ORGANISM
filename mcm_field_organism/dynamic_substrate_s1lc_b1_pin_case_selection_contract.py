"""Static S1-LC selection contract for the finite B1/P_IN case."""
...
class DTS1S1LCB1PINCaseSelectionContractError(ValueError):
    """Raised when the finite S1-LC case selection is weakened."""


S1_LC_CONTRACT_ID = "dynamic-substrate.b1-pin-case-selection.s1lc.v1"
S1_LC_SOURCE_S1LB_DIGEST = "d5ebc93d6521d384d0087ea2601df52a5b0ebe2cacea34d3b920966b326c54ed"
S1_LC_CASE_ID = "C04"
S1_LC_SEQUENCE_KEYS = ("P_IN_RECOVERY_ON", "P_IN_RECOVERY_OFF")
S1_LC_TARGET_REPLICA_IDS = (
    "B1:P_IN_RELEASE_REUSE:r2",
    "B1:P_IN_RELEASE_REUSE:r4",
    "B1:P_IN_RELEASE_REUSE:r8",
)
S1_LC_TARGET_CASE_RECORD = next(row for row in S1_JX_CASE_RECORDS if row[0] == S1_LC_CASE_ID)
S1_LC_TARGET_REPLICA_RECORDS = tuple(row for row in S1_JX_REPLICA_RECORDS if row[0] in S1_LC_TARGET_REPLICA_IDS)
S1_LC_SEQUENCE_RECORDS = tuple(row for row in S1_JX_SEQUENCE_RECORDS if row[0] in S1_LC_SEQUENCE_KEYS)
S1_LC_FRESH_STATE_RECORD = next(
    row for row in S1_JZ_FRESH_STATE_RECORDS
    if row[0] == "B1" and row[1] == "THREE_NODE_OPEN_LINE"
)
S1_LC_FRESH_FIELD_DIGEST = "55278f3e39a856f4846a45f65a0e4289b486b1a3bb5c32deb5eede0828e3c2d8"
S1_LC_FRESH_PRIVATE_STATE_DIGEST = "7f9afbe3dccf65514ba8dd5b61d6c24b5113c068655a05861fe1415ade374ee1"
S1_LC_FRESH_START_AND_CARRY_RULES = (
    "r2-r4-and-r8-each-use-two-independent-corrected-B1-fresh-states-one-per-sequence",
    "P_IN_RECOVERY_ON-and-P_IN_RECOVERY_OFF-each-start-once-from-bit-identical-fresh-field-and-fixed-adapter-state",
...
S1_LC_OUTPUT_ACCEPTANCE_RULES = (
    ...
    "all-three-replicas-must-pass-before-C04-may-be-composed-in-a-later-stage",
)
S1_LC_SELECTION_RATIONALE = (
    "C04-is-the-next-canonical-registered-case-after-technically-complete-C01-C02-C03-C05-C06-and-C07",
    "C04-binds-the-model-neutral-recovery-on-versus-recovery-off-history-to-the-fixed-B1-adapter",
    "selection-does-not-predict-sign-threshold-release-reuse-baseline-closure-or-candidate-outcome",
)
S1_LC_FORBIDDEN_SCOPE = (
    "no-runner-initializer-adapter-or-output-implementation",
    "no-replica-sequence-interval-retry-or-repeat-execution",
    "no-C04-output-or-24-case-matrix-publication",
    "no-B2-P_IN-or-other-role-profile-selection",
    "no-release-reuse-baseline-candidate-runtime-or-research-judgment",
)
S1_LC_DECISION = "B1_PIN_C04_SELECTED_THREE_REFINEMENT_TWO_SEQUENCE_DUAL_DIGEST_TWENTY_FOUR_CALL_CONTRACT_BOUND_NO_EXECUTION"
...
class DTS1S1LCB1PINCaseSelectionContract:
    contract_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.contract_id != S1_LC_CONTRACT_ID
            or self.source_s1lb_digest != S1_LC_SOURCE_S1LB_DIGEST
            or self.target_case_record != S1_LC_TARGET_CASE_RECORD
            or self.target_replica_ids != S1_LC_TARGET_REPLICA_IDS
            or self.target_replica_records != S1_LC_TARGET_REPLICA_RECORDS
            or self.sequence_records != S1_LC_SEQUENCE_RECORDS
            or self.corrected_fresh_state_record != S1_LC_FRESH_STATE_RECORD
            or self.fresh_field_digest != S1_LC_FRESH_FIELD_DIGEST
            or self.fresh_private_state_digest != S1_LC_FRESH_PRIVATE_STATE_DIGEST
            ...
            or self.fresh_start_and_carry_rules != S1_LC_FRESH_START_AND_CARRY_RULES
            or self.execution_budget != S1_LC_EXECUTION_BUDGET
            or self.output_acceptance_rules != S1_LC_OUTPUT_ACCEPTANCE_RULES
            or self.selection_rationale != S1_LC_SELECTION_RATIONALE
            or self.forbidden_scope != S1_LC_FORBIDDEN_SCOPE
            ...
            or self.decision != S1_LC_DECISION
            or self.contract_digest != _digest(payload)
        ):
            raise DTS1S1LCB1PINCaseSelectionContractError("S1-LC weakened the finite B1/P_IN case selection")


def build_dts1_s1lc_b1_pin_case_selection_contract() -> DTS1S1LCB1PINCaseSelectionContract:
    """Select and bind C04 without implementing or executing its replicas."""

    source = build_dts1_s1lb_b2_pik_case_output_contract()
    if (
        source.contract_digest != S1_LC_SOURCE_S1LB_DIGEST
        or len(S1_LC_TARGET_REPLICA_RECORDS) != 3
        or tuple(row[0] for row in S1_LC_TARGET_REPLICA_RECORDS) != S1_LC_TARGET_REPLICA_IDS
        or tuple(row[0] for row in S1_LC_SEQUENCE_RECORDS) != S1_LC_SEQUENCE_KEYS
        or tuple(row[3] for row in S1_LC_SEQUENCE_RECORDS) != (4, 4)
        or tuple(row[5] for row in S1_LC_SEQUENCE_RECORDS) != ((4,), (4,))
        or S1_LC_FRESH_STATE_RECORD[6] != S1_LC_FRESH_FIELD_DIGEST
        or S1_LC_FRESH_STATE_RECORD[8] != S1_LC_FRESH_PRIVATE_STATE_DIGEST
    ):
        raise DTS1S1LCB1PINCaseSelectionContractError("registered C04 evidence or corrected B1 fresh state differs")
    values = {
        "contract_id": S1_LC_CONTRACT_ID,
        "source_s1lb_digest": source.contract_digest,
        "target_case_record": S1_LC_TARGET_CASE_RECORD,
        "target_replica_ids": S1_LC_TARGET_REPLICA_IDS,
        "target_replica_records": S1_LC_TARGET_REPLICA_RECORDS,
        "sequence_records": S1_LC_SEQUENCE_RECORDS,
        "corrected_fresh_state_record": S1_LC_FRESH_STATE_RECORD,
        "fresh_field_digest": S1_LC_FRESH_FIELD_DIGEST,
        "fresh_private_state_digest": S1_LC_FRESH_PRIVATE_STATE_DIGEST,
        ...
        "fresh_start_and_carry_rules": S1_LC_FRESH_START_AND_CARRY_RULES,
        "execution_budget": S1_LC_EXECUTION_BUDGET,
        "output_acceptance_rules": S1_LC_OUTPUT_ACCEPTANCE_RULES,
        "selection_rationale": S1_LC_SELECTION_RATIONALE,
        "forbidden_scope": S1_LC_FORBIDDEN_SCOPE,
        "target_replica_count": 3,
        ...
        "decision": S1_LC_DECISION,
    }
    return DTS1S1LCB1PINCaseSelectionContract(**values, contract_digest=_digest(values))
