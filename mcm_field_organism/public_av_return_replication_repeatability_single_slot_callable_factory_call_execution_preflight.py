"""Locked execution preflight for one callable-factory call acceptance."""
...
from .public_av_return_replication_repeatability_single_slot_callable_factory_call_order_acceptance import (
...
SINGLE_SLOT_CALLABLE_FACTORY_CALL_EXECUTION_PREFLIGHT_ID = (
    "public.av.nasa-earthrise.return-replication."
    "repeatability-single-slot-callable-factory-call-execution-preflight.v1"
)


class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionPreflightError(ValueError):
    pass


@dataclass(frozen=True)
class PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionPreflight:
    preflight_id: str
    selected_repeat_index: int
    callable_factory_call_order_acceptance_id: str
    callable_factory_call_order_id: str
    call_execution_candidate_step: PublicAVReturnReplicationRepeatabilitySingleSlotFactoryExecutionStep
    untouched_gate_factory_step: PublicAVReturnReplicationRepeatabilitySingleSlotFactoryExecutionStep
    call_execution_candidate_step_id: str
    ...
    source_id: str
    positive_call_order_acceptance_bound: bool
    exactly_one_call_execution_candidate_bound: bool
    ...
    callable_factory_call_execution_preflight_complete: bool
    ...
    def __post_init__(self) -> None:
        error = PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionPreflightError
        ...
        indexed_ids = (
            self.preflight_id, self.callable_factory_call_order_acceptance_id,
            self.callable_factory_call_order_id, self.call_execution_candidate_step_id,
        ...
        if not all(identifier.endswith(suffix) for identifier in indexed_ids):
            raise error("call execution preflight identities must match selected repeat index")
        candidate = self.call_execution_candidate_step
        gate = self.untouched_gate_factory_step
        if not (
            candidate.step_index == 1 and candidate.role == "callable_factory"
            and candidate.step_id == self.call_execution_candidate_step_id
            ...
            raise error("call execution candidate must be the unconsumed callable factory call")
        if not (
        ...
        required = (
            self.positive_call_order_acceptance_bound,
            self.exactly_one_call_execution_candidate_bound,
            ...
            self.callable_factory_call_execution_preflight_complete,
        )
        forbidden = (
        ...
        if not all(required) or any(forbidden):
            raise error("callable factory call execution preflight remains fully non-executable")
        object.__setattr__(self, "other_slots_unselected", tuple(self.other_slots_unselected))


def preflight_public_av_return_replication_repeatability_single_slot_callable_factory_call_execution(
    ...
    error = PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionPreflightError
    if not isinstance(acceptance, PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallOrderAcceptance):
        raise error("single-slot callable factory call order acceptance has the wrong type")
    required = (
        acceptance.positive_callable_factory_call_order_accepted,
        acceptance.exactly_one_future_callable_call_accepted,
        acceptance.callable_call_one_time_accepted,
        acceptance.callable_call_unexecuted_accepted,
        ...
        acceptance.callable_factory_call_order_acceptance_complete,
    )
    forbidden = (
    ...
    if not all(required) or any(forbidden):
        raise error("one complete fresh callable factory call order acceptance is required")
    ...
    return PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionPreflight(
        preflight_id=f"{SINGLE_SLOT_CALLABLE_FACTORY_CALL_EXECUTION_PREFLIGHT_ID}.repeat-{index}.v1",
        callable_factory_call_order_acceptance_id=acceptance.acceptance_id,
        call_execution_candidate_step=acceptance.accepted_callable_call_step,
        call_execution_candidate_step_id=acceptance.accepted_callable_call_step_id,
...
def execute_public_av_return_replication_repeatability_single_slot_callable_factory_call_execution_preflight(
    preflight: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionPreflight,
) -> None:
    del preflight
    raise PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionPreflightError(
        "factory call execution is not released by the locked callable factory call execution preflight"
    )


def public_av_return_replication_repeatability_single_slot_callable_factory_call_execution_preflight_to_jsonable(
    preflight: PublicAVReturnReplicationRepeatabilitySingleSlotCallableFactoryCallExecutionPreflight,
) -> dict[str, Any]:
